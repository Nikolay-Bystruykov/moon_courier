...
<section class="panel p-4">
    ...
    <svg viewBox="0 0 {{ $map['width'] * $cell }} {{ $map['height'] * $cell }}"
         class="w-full"
         role="img"
         aria-label="Карта лунной поверхности с базой и аванпостами">
        ...
        <g>
            ...
            <text x="{{ $base['x'] * $cell + $cell / 2 }}" y="{{ $base['y'] * $cell - 5 }}"
                  ...
                  font-size="9" font-family="IBM Plex Mono, monospace" letter-spacing="1.5">БАЗА</text>
        </g>
        ...
        @foreach ($outposts as $outpost)
            ...
            <g class="cursor-pointer" @click="selectOutpost({{ $outpost['id'] }})"
               ...
               @keydown.enter="selectOutpost({{ $outpost['id'] }})">
                ...
                <text x="{{ $cx }}" y="{{ $outpost['y'] * $cell + $cell + 11 }}"
    ...
    <ul class="mt-4 flex flex-wrap gap-x-5 gap-y-2 border-t border-edge pt-3">
